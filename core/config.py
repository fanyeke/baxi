...
All core layer modules (api, services, adapters) should import from this module.
"""
...
# Project root (parent of core/ directory)
...
QODER_CAPABILITIES_FILE = os.path.join(CONFIG_DIR, 'qoder_capabilities.yml')
...
def get_env_or_raise(key: str, default: str = "") -> str:
    """Get an environment variable, raising if missing or placeholder.

    If default is provided and the variable is not set, returns the default.
    """
...
if __name__ == '__main__':
    ...
    logger.info("  SYSTEM_DIR=%s", SYSTEM_DIR)
    logger.info("  ADS_DIR=%s", ADS_DIR)
    logger.info("  AIP_DIR=%s", AIP_DIR)
    logger.info("  FEISHU_DIR=%s", FEISHU_DIR)
