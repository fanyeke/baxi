...
// If no role is specified, default to "analyst" access.
...
// NewOntologyRepo creates a new OntologyRepo.
func NewOntologyRepo() *OntologyRepo {
	return &OntologyRepo{}
}

// resolveRole extracts the role from context, defaulting to "analyst".
...
// tableAccessible checks if the given role has access to the table.
...
// resolveLimit enforces LIMIT bounds: default 1000, max 10000.
...
func (r *OntologyRepo) GetObjectByID(ctx context.Context, pool *pgxpool.Pool, objectType, objectID string) (*ObjectInstance, error) {
	...
	rows, err := pool.Query(ctx, query, objectID)
	if err != nil {
...
func (r *OntologyRepo) GetObjectMetrics(ctx context.Context, pool *pgxpool.Pool, objectType, objectID string) (*ObjectMetrics, error) {
	...
	if aggMetrics, hasMetrics := metricColumns[objectType]; hasMetrics {
		...
		row := pool.QueryRow(ctx, query, objectID)
