...
MCP (Model Context Protocol) 服务器，通过 stdio 传输协议暴露 Baxi 核心能力。基于 `github.com/mark3labs/mcp-go` 框架，支持 Pi Agent 等 MCP 兼容客户端连接。注册 31 个工具，按 11 个业务域分组。
...
| `tools_decision.go` | 6 个决策域工具：create_decision_case, decide, resolve_case, list_cases, get_case, list_proposals |
...
| `tools_review.go` | 5 个审核域工具：approve_proposal, reject_proposal, cancel_proposal, get_proposal_by_id, list_review_records |
...
| `tools_ontology.go` | 4 个本体域工具：describe_ontology, get_object, get_linked_objects, execute_action |
| `tools_sandbox.go` | 4 个沙盘域工具：create_sandbox, add_to_sandbox, compare_sandboxes, get_sandbox |
| `tools_schema.go` | 2 个 Schema 域工具：list_action_schemas, get_action_schema |
...
| 验证工具注册 | `server_test.go` | `TestServerToolRegistration` 维护 31 个工具名白名单 |
...
| resolve_case | decision | case_id(必填), resolution(必填), comment | 解决决策 Case |
...
| cancel_proposal | review | proposal_id(必填), reason | 取消提案 |
| get_proposal_by_id | review | proposal_id(必填) | 查询提案详情 |
| list_review_records | review | proposal_id(必填), limit, offset | 分页查询审核记录 |
...
| describe_ontology | ontology | (无参数) | 描述所有注册对象类型 |
| get_object | ontology | object_type(必填), object_id(必填) | 按类型和 ID 查询对象 |
| get_linked_objects | ontology | object_type(必填), object_id(必填), link_name, max_depth | 查询关联对象 |
| execute_action | ontology | object_type(必填), object_id(必填), action_type(必填), params | 在对象上执行动作 |
| create_sandbox | sandbox | case_id(必填) | 创建沙盘 |
| add_to_sandbox | sandbox | sandbox_id(必填), proposal_id(必填) | 添加提案到沙盘 |
| compare_sandboxes | sandbox | sandbox_id_1(必填), sandbox_id_2(必填) | 比较两个沙盘差异 |
| get_sandbox | sandbox | sandbox_id(必填) | 查询沙盘详情 |
| list_action_schemas | schema | (无参数) | 列出所有动作 Schema |
| get_action_schema | schema | action_type(必填) | 查询指定动作 Schema |
